...
function reenableGain(cameraName) {
    sendToPeer(orchestrator, '{"topic":"cameraControl","message":{"action":"disableGain", "camera":"' + cameraName + '"}}');
}
...
function handlePeerMessage(peer_id, data) {
    ...
    if (data.search("offer") != -1) {
        ...
        localPeerConnection.createAnswer(function(description) {
            ...
            if (ocupusCameraBitRates[other_peers[peer_id]]) {
                description.sdp = setBitRate(description.sdp, ocupusCameraBitRates[other_peers[peer_id]]);
            } else {
...
var tx_data = d3.range(150).map(function() {
    return 0
});
...
function chart(domain, interpolation, tick) {
    ...
    svg.append("g")
        .attr("class", "y axis")
        .call(d3.svg.axis().tickFormat(function(d) {
            return d.toFixed(1) + " mbps";
        }).scale(y).ticks(5).orient("left"));
